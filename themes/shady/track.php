...
<div class="container">
  ...
  <div class="buttons">
    ...
    <?php if ( !$track["is_paid"] ) : ?>
      <div class="button purchase_handle"
		   data-title="<?php $loader->lorem->eturn( 'purchase_song', [ "uc" => true ] ) ?>"
		   data-item-title="<?php echo $loader->secure->escape( $track["artist_name"] . " - " . $track["title"] ); ?>"
		   data-item-price="<?php echo $track["price"]; ?>"
		   data-fund="<?php echo $loader->user->data["fund"]; ?>"
		   data-item-hook="<?php echo $track["hash"]; ?>"
		   data-item-type="track">
      <a class="btn btn-secondary more"><span class="mdi mdi-cart"></span> <?php $loader->lorem->eturn( "buy_for", [ "uc" => true, "params" => [ "price" => $loader->general->display_price( $track["price"], true ) ] ] ); ?></a></div>
    <?php endif; ?>

    <div class="button like_btn like_btn_<?php echo $track["hash"] . ( $track["is_liked"] ? " liked" : "" ); ?> m_like" data-hook="<?php echo $track["hash"]; ?>">
      <a class="btn btn-secondary"><span class="mdi mdi-heart"></span> <span class="text"><?php $loader->lorem->eturn( $track["is_liked"] ? "unlike" : "like", [ "uc" => true ] ) ?></span></a>
    </div>

      <div class="button">
        <a
		   data-title = "<?php $loader->lorem->eturn( "share_this", [ "uc" => true, "params" => [ "target" => $loader->lorem->turn( "track", ["uc"=>true] ) ] ] ) ?>"
		   data-url = "<?php $loader->ui->eurl( "track", $track["url"] ) ?>"
		   data-image = "<?php echo $track["cover_addr"] ?>"
		   data-name = "<?php echo $loader->secure->escape( $track["artist_name"] . " - " . $track["title"] ) ?>"
       data-type = "track"
		   class="share_handle btn btn-secondary"
        ><span class="mdi mdi-share"></span> <?php $loader->lorem->eturn( "share", [ "uc" => true ] ); ?></a>
      </div>

      <div class="button more_btn button_more" data-type="track" data-hash="<?php echo $track["hash"]; ?>" >
        <a class="btn btn-secondary more"><span class="mdi mdi-dots-horizontal"></span> <?php $loader->lorem->eturn( "more", [ "uc" => true ] ); ?></a>
      </div>
